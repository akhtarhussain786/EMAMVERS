...
if ($action === 'get_user_360') {
    ...
    try {
        ...
        ajaxOk([
            ...
            'performance' => [
                'total_attempts'      => intval($agg['total_attempts'] ?? 0),
                'completed_attempts'  => intval($agg['completed_attempts'] ?? 0),
                'avg_score'           => round(floatval($agg['avg_score'] ?? 0), 1),
                'max_score'           => round(floatval($agg['max_score'] ?? 0), 1),
                'avg_accuracy'        => round(floatval($agg['avg_accuracy'] ?? 0), 1),
                'best_air_rank'       => !empty($agg['best_air_rank']) ? intval($agg['best_air_rank']) : null,
                'best_state_rank'     => !empty($agg['best_state_rank']) ? intval($agg['best_state_rank']) : null,
                'total_correct'       => intval($agg['total_correct'] ?? 0),
                'total_wrong'         => intval($agg['total_wrong'] ?? 0),
                'total_unattempted'   => intval($agg['total_unattempted'] ?? 0),
                'total_time_spent'    => intval($agg['total_time_spent'] ?? 0),
        ...
        ajaxErr('Database query failed: ' . $ex->getMessage());
    }
}
...
if ($action === 'grant_subscription') {
    ...
    try {
        ...
        ajaxErr('Failed to grant subscription: ' . $ex->getMessage());
    }
}
...
if ($action === 'send_notification') {
    ...
    try {
        ...
        ajaxErr('Failed to send notification: ' . $ex->getMessage());
    }
}
